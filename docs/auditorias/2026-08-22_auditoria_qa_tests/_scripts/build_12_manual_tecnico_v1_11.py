"""
Build 12 — Manual Técnico v1.11 (QA Total 2026-08-22)
Genera: docs/documentacion_oficial/12_Manual_Tecnico_Custodio_RAT_Manager_v1.11.docx
Cambios v1.11:
- auth.py: POST /auth/users status_code=201
- tkt_solicitud_derecho.py: PATCH resuelto con metodo_verificacion_identidad en body
- encrypt_existing_bytea.py: ENCRYPTION_KEY sin fallback a settings
- Sprint A: soft delete RAT, test IL gate, EIPD gate, datos NNA, mutex anon/seudo
- Sprint B: respuesta_texto obligatoria, SLA alert 72h, SLA plazo retención
- Sprint UX: banner NNA, politica transparencia editable
"""
import os
import sys
...
OUT_FILE = os.path.join(OUT_DIR, "12_Manual_Tecnico_Custodio_RAT_Manager_v1.11.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.9", "Julio 2026", "_IDOR multi-tenant get_rat_for_user() en 6 endpoints RAT. base_legal_valida strict. ConsentimientoAlert. Homologacion campos RAT. PDF titulos seccion._"),
        ("1.10", "Agosto 2026", "_Sprint A+B+UX: soft delete RAT, test IL gate, EIPD gate, NNA base legal, mutex anon/seudo, respuesta_texto obligatoria, SLA alerts, politica transparencia editable._"),
        ("1.11", "Agosto 2026", "_QA Total (2026-08-22): POST /auth/users → 201. PATCH TKT resuelto con metodo_verificacion_identidad en body. encrypt_existing_bytea ENCRYPTION_KEY estricta. 732 tests passing, 0 fallos._"),
    ])
    add_toc(doc)

    doc.add_heading("1. Cambios técnicos en v1.11", level=1)
    cambios_v18 = [
        ...
        ["POST /auth/users → 201", "ALTO", "routes/auth.py:129",
         "Agregado status_code=201 al endpoint de creacion de usuario. Antes retornaba 200 por default FastAPI."],
        ["PATCH TKT resuelto: metodo_verificacion en body", "ALTO", "routes/tkt_solicitud_derecho.py:317",
         "Condicion corregida: 'not ticket.metodo_verificacion_identidad and not data.metodo_verificacion_identidad'. Antes el handler verificaba solo el campo BD, ignorando el body del PATCH."],
        ["encrypt_existing_bytea: ENCRYPTION_KEY estricta", "MEDIO", "scripts/migration/encrypt_existing_bytea.py:76",
         "Eliminado fallback a settings.ENCRYPTION_KEY. El script exige que ENCRYPTION_KEY este explicitamente en el entorno. Evita usar la key de produccion en migraciones no supervisadas."],
        ["Sprint A: soft delete RAT (Art. 19)", "ALTO", "services/rat_crud.py, routes/rats.py",
         "delete_rat() asigna deleted_at. get_rats() y get_rat() filtran deleted_at IS NULL. RATs aprobados no se pueden eliminar (409)."],
        ["Sprint A: EIPD gate aprobacion (Art. 15 bis)", "ALTO", "services/rat_crud.py (aprobar_rat)",
         "validar_eipd_obligatoria() se invoca antes de cambiar estado a APROBADO. Bloquea aprobacion sin EIPD cuando datos_sensibles=True o transferencia_internacional=True."],
        ["Sprint B: respuesta_texto obligatoria (Art. 12)", "ALTO", "services/ticket_service.py",
         "Bloquea PATCH→resuelto sin respuesta_texto no vacia."],
        ["Sprint UX: politica transparencia editable (Art. 14 ter)", "MEDIO", "routes/transparencia.py, models/politica_transparencia.py",
         "PUT /transparencia/{company_id} (autenticado). overrides_json permite personalizar items. Hash SHA-256 recalculado en cada guardado."],
    ]
