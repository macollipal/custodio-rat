"""
Build 06 — Arquitectura de Software v1.7 (Post Sprint 1 + Sprint 2 ARCO)
Genera: docs/documentacion_oficial/06_Arquitectura_Software_Custodio_RAT_Manager_v1.7.docx
Cambios v1.7:
- Sprint 1: FormADMIN ARCO QW1-QW10 (ADR-19) — validación RUT, debounce, doble input, tooltips
- Sprint 2: Task SLA_ALERT_T2 pattern (ADR-20), Export service (ADR-21), GitHub Actions (ADR-22)
--- (v1.6-BETA ---)
- RatDetailModal con tabs Ver/Editar (componente React con useReducer)
- Drawer responsive: 5 sizes, maxHeight 92vh, hasHeader conditional
- Dashboard clickable: RatDetailModal in-page con onDuplicate/onDelete
- useRef pattern para callbacks estables (auditLogs, Date.now)
"""
import os
import sys
...
OUT_FILE = os.path.join(OUT_DIR, "06_Arquitectura_Software_Custodio_RAT_Manager_v1.7.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.7", "Junio 2026", "_Sprint 1: FormADMIN ARCO QW1-QW10 (ADR-19). Sprint 2: Task SLA_ALERT_T2 pattern (ADR-20), Export service (ADR-21), GitHub Actions (ADR-22)._"),
    ])
    add_toc(doc)
