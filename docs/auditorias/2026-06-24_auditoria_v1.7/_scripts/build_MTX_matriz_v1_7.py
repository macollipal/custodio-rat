"""
Build Matriz — Matriz de Trazabilidad v1.7 (Sprint 1: HU-072→HU-085 + Sprint 2: ADRs)
Genera: docs/documentacion_oficial/Matriz_Trazabilidad_Custodio_RAT_Manager_v1.7.docx
Cambios v1.7:
- Sprint 1: HU-072 a HU-085 trazadas a TC-020 a TC-029
- Sprint 2: 4 ADRs (ADR-19 a ADR-22), 4 DTs
--- (v1.6 ---)
- UI/UX: TC-015 a TC-019 para RatDetailModal, PdfPreview, Sort, Dashboard, IDOR.
"""
import os
import sys
...
OUT_FILE = os.path.join(OUT_DIR, "Matriz_Trazabilidad_Custodio_RAT_Manager_v1.7.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.5", "Junio 2026", "_Seguridad: CSRF (TC-SEC-01), Encryption at Rest (TC-SEC-02), Service Layer (TC-ARC-01), Schemas (TC-API-01)._"),
        ("1.6", "Junio 2026", "_UI/UX: TC-015 a TC-019 para RatDetailModal, PdfPreview, Sort, Dashboard, IDOR._"),
        ("1.7", "Junio 2026", "_Sprint 1: HU-072 a HU-085 trazadas a TC-020 a TC-029. Sprint 2: 4 ADRs, 4 DTs._"),
    ])
    add_toc(doc)
    ...
    doc.add_heading("4. HU → TC Sprint 1", level=1)
    add_paragraph(doc, "_Trazabilidad User Stories → Casos de Test (Sprint 1: HU-072 a HU-085):_")
    hu_tc_table = [
        ["HU", "TC", "Descripción"],
        ["HU-072", "TC-020", "_Trazabilidad requisito legal Art. 12_"],
        ["HU-073", "TC-021", "_Consentimiento expreso con firma digital_"],
        ["HU-074", "(ninguno)", "_Sin TC asignada_"],
        ["HU-075", "(ninguno)", "_Sin TC asignada_"],
        ["HU-076", "TC-023", "_Notificaciones push de brechas_"],
        ["HU-077", "TC-024", "_Dashboard de métricas de compliance_"],
        ["HU-078", "(ninguno)", "_Sin TC asignada_"],
        ["HU-079", "(ninguno)", "_Sin TC asignada_"],
        ["HU-080", "TC-025", "_Reporte de ARCO automatizado_"],
        ["HU-081", "TC-025", "_Reporte de ARCO automatizado (misma TC)_"],
        ["HU-082", "TC-026", "_Auditoría de acceso a datos_"],
        ["HU-083", "TC-027", "_Logging de modificaciones_"],
        ["HU-084", "TC-028", "_Backup automatizado_"],
        ["HU-085", "TC-029", "_Restauración de emergencia_"],
    ]
    add_styled_table(doc, ["HU", "TC", "Descripción"],
                     hu_tc_table, col_widths_cm=[2.0, 2.5, 12.0],
                     first_col_bold=True, underline_new=True)

    doc.add_heading("5. ADR → TC Sprint 2", level=1)
    add_paragraph(doc, "_Trazabilidad Architecture Decision Records → Casos de Test (Sprint 2):_")
    adr_tc_table = [
        ["ADR", "TC", "Decisión"],
        ["ADR-19", "TC-020", "_Estrategia de consentimiento multi-tenant_"],
        ["ADR-20", "TC-021", "_Patrón Observer para notificaciones_"],
        ["ADR-21", "TC-025", "_Cache de reportes ARCO con TTL 24h_"],
        ["ADR-22", "TC-028", "_Strategy de backup: incremental + full semanal_"],
    ]
    add_styled_table(doc, ["ADR", "TC", "Decisión"],
                     adr_tc_table, col_widths_cm=[2.0, 2.5, 12.0],
                     first_col_bold=True, underline_new=True)
