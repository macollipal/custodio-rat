"""
Build 03 — Historias de Usuario v1.10 (Post Iter 11 + Iter 12 audit-loop)
Genera: docs/documentacion_oficial/03_Historias_Usuario_Custodio_RAT_Manager_v1.10.docx
Cambios v1.10:
...
OUT_FILE = os.path.join(OUT_DIR, "03_Historias_Usuario_Custodio_RAT_Manager_v1.10.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.9", "Julio 2026", "_IDOR multi-tenant en 6 endpoints RAT (HU-098). base_legal_valida strict (HU-099). ConsentimientoAlert en handleSave (HU-100). Homologación orden campos RAT (HU-101). PDF con títulos sección (HU-102). Encoding UTF-8 fix (HU-103)._"),
    ])
    add_toc(doc)
    ...
    doc.add_heading("3.19 Fixes v1.10: IDOR multi-tenant, encoding, homologación y PDF (Julio 2026)", level=2)
    hus_v19 = [
        ["HU-098", "EP-02", "RF-163", "CRÍTICO", "M", "_IDOR multi-tenant en 6 endpoints RAT: get_rat_for_user() retorna 404 si empresa no coincide, superadmin accede a todos_"],
        ["HU-099", "EP-02", "RF-164", "ALTO", "S", "_base_legal_valida strict: validación contra enum taxativo (6 opciones)_"],
        ["HU-100", "EP-11", "RF-165", "ALTO", "S", "_ConsentimientoAlert: listarConsentimientos() antes de guardar si datos_sensibles=True_"],
        ["HU-101", "EP-02", "RF-166", "ALTO", "L", "_Homologación orden campos RAT entre RatDetailView, RatEditForm, RatWizard y PDF (5 pasos canónicos)_"],
        ["HU-102", "EP-06", "RF-167", "MEDIO", "M", "_PDF con títulos de sección (PASO 1— IDENTIFICACIÓN, etc.) con fondo COLOR_PRIMARIO y alertas rojas_"],
        ["HU-103", "EP-02", "RF-168", "MEDIO", "S", "_Encoding UTF-8 corregido en backend: ALERTAS_AUDITORIA, regex, base_legal_valida, conftest.py, main.py_"],
    ]
    add_styled_table(doc, ["ID", "EP", "RF", "Prioridad", "Complex", "Título"], hus_v19,
                     col_widths_cm=[1.5, 1.0, 1.5, 1.5, 1.5, 10.5], first_col_bold=True, underline_new=True)

    doc.add_heading("4. Épicas del producto (completo)", level=1)
    epicas_full = [
        ["EP-01", "Autenticación y multi-tenancy", "HU-001 a HU-009"],
        ["EP-02", "Gestión del RAT", "HU-010 a HU-040, HU-086 a HU-089, HU-098 a HU-099, HU-101, HU-103"],
        ...
        ["EP-06", "Reportes y exportaciones", "HU-070 a HU-080, HU-102"],
        ...
        ["EP-11", "Consentimientos y EIPD", "HU-056 a HU-059, HU-100"],
