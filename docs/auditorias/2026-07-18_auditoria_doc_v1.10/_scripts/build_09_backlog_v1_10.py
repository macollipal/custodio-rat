"""
Build 09 — Backlog de Producto v1.10 (Post Iter 11 + Iter 12)
Genera: docs/documentacion_oficial/09_Backlog_Producto_Custodio_RAT_Manager_v1.10.docx
Cambios v1.10:
...
OUT_FILE = os.path.join(OUT_DIR, "09_Backlog_Producto_Custodio_RAT_Manager_v1.10.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.9", "Julio 2026", "_IDOR multi-tenant en 6 endpoints RAT (DT-ITER13-01). base_legal_valida strict (DT-ITER13-02). ConsentimientoAlert (DT-ITER13-03). Homologación orden campos RAT (DT-ITER13-04). PDF títulos sección (DT-ITER13-05). Encoding UTF-8 fix (DT-ITER13-06)._"),
    ])
    add_toc(doc)

    doc.add_heading("1. Vista general del backlog", level=1)
    add_paragraph(doc, "_Total: 49 items. Estado al 05-Jul-2026: 41 completados, 3 en desarrollo, 5 pendientes. "
                      "Prioridad: Critical (P0) > High (P1) > Medium (P2) > Low (P3)._")

    doc.add_heading("2. Items cerrados en v1.10 (Iter 11 + Iter 12)", level=1)
    items_cerrados = [
        ...
        ["DT-ITER13-01", "P1", "Bug", "_IDOR multi-tenant: get_rat_for_user() en 6 endpoints RAT (404 si empresa no coincide)_",
         "routes/rats.py", "CERRADO"],
        ["DT-ITER13-02", "P2", "Bug", "_base_legal_valida strict contra enum taxativo (6 opciones)_",
         "schemas/rat.py", "CERRADO"],
        ["DT-ITER13-03", "P2", "Feature", "_ConsentimientoAlert: listarConsentimientos() si datos_sensibles=True_",
         "RatEditForm.tsx handleSave()", "CERRADO"],
        ["DT-ITER13-04", "P2", "Bug", "_Homologación orden campos RAT entre RatDetailView, RatEditForm, RatWizard y PDF_",
         "RatDetailView, RatEditForm, RatWizard, export_service", "CERRADO"],
        ["DT-ITER13-05", "P3", "Feature", "_PDF con títulos de sección y alertas rojas_",
         "export_service.py", "CERRADO"],
        ["DT-ITER13-06", "P3", "Bug", "_Encoding UTF-8 corregido: ALERTAS_AUDITORIA, regex, conftest.py, main.py_",
         "rat_service.py, main.py, conftest.py", "CERRADO"],
    ]
    ...
    doc.add_heading("3. Items pendientes (QW-ITER14)", level=1)
    items_pendientes = [
        ["QW-ITER14-01", "P2", "Feature", "Paginación en listados >100 registros (RAT/ARCO/Brechas)", "Alto esfuerzo"],
        ["QW-ITER14-02", "P3", "Feature", "Retry logic en OCI uploads (resilience)", "Medio esfuerzo"],
        ["QW-ITER14-03", "P2", "Feature", "Logs de auditoría en tabla audit_log (Art. 28 Ley 21.719)", "Medio esfuerzo"],
        ["QW-ITER14-04", "P3", "Feature", "ALTER TABLE categoria_titulares SET NOT NULL (breaking change BD)", "Medio esfuerzo"],
    ]
