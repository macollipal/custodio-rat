"""
Build 02 — Levantamiento de Requisitos v1.10 (Post Iter 11 + Iter 12 audit-loop)
Genera: docs/documentacion_oficial/02_Requisitos_Custodio_RAT_Manager_v1.10.docx
Cambios v1.10:
...
OUT_FILE = os.path.join(OUT_DIR, "02_Requisitos_Custodio_RAT_Manager_v1.10.docx")
...
def build():
    ...
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.9", "Julio 2026", "_IDOR multi-tenant en 6 endpoints (RF-163). base_legal_valida strict (RF-164). ConsentimientoAlert (RF-165). Homologación orden campos RAT (RF-166). PDF con títulos sección (RF-167). Encoding UTF-8 fix (RF-168). Código muerto eliminado (RF-169)._"),
    ])
    add_toc(doc)
    ...
    doc.add_heading("3.18 Fixes v1.10: IDOR multi-tenant, encoding, homologación y PDF (Julio 2026)", level=2)
    rf_v19 = [
        ["RF-163", "CRÍTICO", "Implementado", "_IDOR multi-tenant en 6 endpoints RAT: GET /rats/{id}, PUT /rats/{id}, DELETE /rats/{id}, POST /rats/{id}/revision, POST /rats/{id}/aprobar, GET /rats/{id}/auditoria. get_rat_for_user() retorna 404 si empresa no coincide (no filtra existencia), superadmin accede a todos._"],
        ["RF-164", "ALTO", "Implementado", "_base_legal_valida strict: validación contra enum taxativo (consentimiento, interes_legitimo, contrato, obligacion_legal, tarea_publica, interes_publico). Antes siempre retornaba v.strip() sin verificar._"],
        ["RF-165", "ALTO", "Implementado", "_ConsentimientoAlert en RatEditForm.handleSave(): listarConsentimientos(company_id, rat.id, true) antes de guardar. Si datos_sensibles=True y no hay consentimiento activo → toast error y no guarda._"],
        ["RF-166", "ALTO", "Implementado", "_Homologación orden campos RAT entre RatDetailView, RatEditForm, RatWizard y PDF: orden canónico de 5 pasos (Identificación → Datos tratados → Finalidad y ley → Almacenamiento → Compliance operativo). Eliminada sección duplicada en RatDetailView._"],
        ["RF-167", "MEDIO", "Implementado", "_PDF con títulos de sección: PASO 1 — IDENTIFICACIÓN, PASO 2 — DATOS TRATADOS, etc. con fondo COLOR_PRIMARIO (#1B3A6B) y texto blanco bold. Alertas rojas (datos sensibles, transferencia internacional, EIPD pendiente) al final de cada ficha._"],
        ["RF-168", "MEDIO", "Implementado", "_Encoding UTF-8 corregido en backend: ALERTAS_AUDITORIA (garabatos → emojis ⚠️🔒🚨📋🌍🤖⚖️⏳📄), regex a├▒o → año, base_legal_valida con tildes, conftest.py rat_base fixture, main.py log seed admin._"],
        ["RF-169", "BAJA", "Implementado", "_Código muerto eliminado: return duplicado rats.py:291, model_dump duplicado rat_service.py:253._"],
    ]
    add_styled_table(doc, ["ID", "Prioridad", "Estado", "Descripción"], rf_v19,
                     col_widths_cm=[1.6, 1.6, 2.0, 12.3], first_col_bold=True, underline_new=True)
