"""
Build 10 — Plan de QA v1.10 (Post Iter 11 + Iter 12)
Genera: docs/documentacion_oficial/10_Plan_QA_Custodio_RAT_Manager_v1.10.docx
Cambios v1.10:
...
OUT_FILE = os.path.join(OUT_DIR, "10_Plan_QA_Custodio_RAT_Manager_v1.10.docx")
...
def build():
    ...
    add_cover(doc, title="PLAN DE QUALITY ASSURANCE",
              subtitle="Estrategia, casos de prueba y métricas · v1.10",
              code=DOC_CODE)
    add_version_control(doc, DOC_CODE, DOC_TITLE, changes=[
        ...
        ("1.9", "Julio 2026", "_TC-039 a TC-043 para IDOR multi-tenant (5 tests), base_legal_valida strict, ConsentimientoAlert, homologación campos RAT, PDF títulos sección._"),
    ])
    add_toc(doc)

    doc.add_heading("1. Resumen ejecutivo", level=1)
    add_paragraph(doc, "_v1.10 (05-Jul-2026): 9 fixes CRITICOS+ALTOS Iter 12. Score: 6.3/10. pytest: 10/10 PASS (test_rat_tier1_tier2.py). TypeScript: 0 errores._")
    add_paragraph(doc, "QA backend: https://custodio-api-qa.vercel.app. QA frontend: https://custodio-qa.vercel.app.")

    doc.add_heading("2. Casos de prueba nuevos (v1.10 — Iter 11+12)", level=1)
    tc_nuevos = [
        ...
        ["TC-039", "CRÍTICO", "Backend", "IDOR: empresa B no puede ver RAT de empresa A → 404",
         "GET /rats/{id} con token empresa B, rat_id de empresa A", "HTTP 404", "pytest"],
        ["TC-040", "CRÍTICO", "Backend", "IDOR: empresa B no puede actualizar RAT de empresa A → 404",
         "PUT /rats/{id} con token empresa B, rat_id de empresa A", "HTTP 404", "pytest"],
        ["TC-041", "CRÍTICO", "Backend", "IDOR: empresa B no puede eliminar RAT de empresa A → 404",
         "DELETE /rats/{id} con token empresa B, rat_id de empresa A", "HTTP 404", "pytest"],
        ["TC-042", "CRÍTICO", "Backend", "IDOR: empresa B no puede auditar RAT de empresa A → 404",
         "GET /rats/{id}/auditoria con token empresa B, rat_id de empresa A", "HTTP 404", "pytest"],
        ["TC-043", "ALTO", "Backend", "IDOR: superadmin sí puede acceder a RAT de cualquier empresa",
         "GET /rats/{id} con token superadmin, rat_id de cualquier empresa", "HTTP 200", "pytest"],
        ["TC-044", "ALTO", "Backend", "base_legal_valida rechaza valor no enumerado",
         "POST /rats/ con base_legal='valor_invalido'", "HTTP 422", "pytest"],
        ["TC-045", "ALTO", "Frontend", "ConsentimientoAlert: toast error si datos_sensibles=True sin consentimiento activo",
         "Editar RAT con datos_sensibles=True, no crear consentimiento antes", "Toast error, no se guarda", "Playwright"],
        ["TC-046", "MEDIO", "Backend", "PDF con títulos de sección (PASO 1 — IDENTIFICACIÓN, etc.)",
         "GET /rats/{id}/export/pdf y verificar contenido", "PDF contiene títulos con COLOR_PRIMARIO", "pytest + pdfminer"],
    ]
    ...
    cobertura = [
        ["Backend pytest", "~240 tests", "pytest + httpx", "8 nuevos en TC-039 a TC-046", "230+ passing"],
