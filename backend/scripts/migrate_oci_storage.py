"""
Migración: Agrega columna archivo_base_legal_storage_url a tabla rats.
Idempotente: si la columna ya existe, no hace nada.

Uso:
    python scripts/migrate_oci_storage.py            # aplica migración
    python scripts/migrate_oci_storage.py --rollback # revierte migración
"""
...
def _print_header(env_label: str, action: str):
    ...
    print(f"=== Migración OCI Storage [{action}] ===")
    print(f"BD: {host}/{db}")
    print(f"Tabla: {TABLE}")
...
def migrate_up():
    ...
    if column_exists(TABLE, COLUMN):
        ...
        print(f"       ERROR - La columna no aparece después del ALTER.")
        sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Migración para agregar columna OCI storage URL a tabla rats."
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--rollback",
        action="store_true",
        help="Revierte la migración (DROP COLUMN).",
    )
